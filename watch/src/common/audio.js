...
function messageForCode(code) {
  ...
  // 202 документирован как «неверные параметры», но перебор форм вызова на
  // устройстве показал другое: им отвечает и вызов вообще без параметров,
  // и притом за две миллисекунды. Параметров, которые можно было бы
  // отклонить, там нет — значит закрыт сам доступ к микрофону. Сообщение
  // должно называть то, что человек может проверить, а не то, что написано
  // в таблице кодов.
  if (code === 202) return "Часы не дают доступ к микрофону"
...
// Потолок снижен вдвое: 96 КБ кадров превращались в 128 тысяч символов
// base64, и в момент отправки в куче лежало и то, и другое. Часы зависают
// именно от таких пиков. Три секунды речи — достаточно для короткого
// вопроса, а длинный всё равно упирается в потолок записи.
var MAX_FRAME_BYTES = 48 * 1024
